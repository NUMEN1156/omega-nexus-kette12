...
pub struct RelayServer { config: RelayConfig, tls_acceptor: TlsAcceptor }

impl RelayServer {
    pub fn new(config: RelayConfig) -> Result<Self, Box<dyn std::error::Error>> {
        Ok(Self { tls_acceptor: build_tls_acceptor(&config)?, config })
    }
    pub async fn run(&self) -> Result<(), Box<dyn std::error::Error>> {
        let listener = TcpListener::bind(&self.config.bind_addr).await?;
        let (routing_tx, _) = broadcast::channel::<(String, RelayMessage)>(256);
        info!("==> kette12-relay (mTLS + protocol + session + subscriptions + routing) listening on {}", self.config.bind_addr);
        loop {
            ...
            tokio::spawn(async move {
                if let Err(e) = handle_client(acceptor, stream, addr, timeout, routing_tx).await { error!("Error handling client {}: {}", addr, e); }
            });
        }
    }
}
...
fn build_tls_acceptor(config: &RelayConfig) -> Result<TlsAcceptor, Box<dyn std::error::Error>> {
    ...
    let key = PrivateKey(rustls_pemfile::pkcs8_private_keys(&mut key_reader)?.into_iter().next().ok_or("No private key found")?);
    let mut ca_reader = BufReader::new(File::open(&config.ca_cert_path)?);
    let mut roots = rustls::RootCertStore::empty();
    for ca in rustls_pemfile::certs(&mut ca_reader)? { roots.add(&Certificate(ca))?; }
    let verifier = AllowAnyAuthenticatedClient::new(roots);
    let cfg = ServerConfig::builder().with_safe_defaults().with_client_cert_verifier(Arc::new(verifier)).with_single_cert(certs, key)?;
    Ok(TlsAcceptor::from(Arc::new(cfg)))
}

fn extract_node_id(cert_der: &[u8]) -> Result<String, Box<dyn std::error::Error>> {
    let (_, cert) = X509Certificate::from_der(cert_der)?;
    if let Some(cn) = cert.subject().iter_common_name().next() { if let Ok(s) = cn.as_str() { return Ok(s.to_string()); } }
    if let Some(sans) = cert.subject_alternative_name()? { for san in &sans.value.general_names { if let GeneralName::DNSName(name) = san { return Ok(name.to_string()); } } }
...
async fn handle_client(
    acceptor: TlsAcceptor, stream: TcpStream, addr: std::net::SocketAddr, session_timeout: Duration,
    routing_tx: broadcast::Sender<(String, RelayMessage)>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let mut tls_stream = acceptor.accept(stream).await?;
    let cert_node_id = { let (_, session) = tls_stream.get_ref(); let certs = session.peer_certificates().ok_or("No peer certificate")?; extract_node_id(&certs.first().ok_or("Empty peer certificate chain")?.0)? };
    let first = time::timeout(session_timeout, protocol::read_message(&mut tls_stream)).await.map_err(|_| "timeout waiting for Handshake")??;
    let (node_id, role) = match first {
        RelayMessage::Handshake { node_id, role } if node_id == cert_node_id => (node_id, role),
        RelayMessage::Handshake { node_id, .. } => { let _ = protocol::write_message(&mut tls_stream, &RelayMessage::Ack { status: "error: node_id mismatch with certificate".into() }).await; return Err(format!("node_id mismatch: {}", node_id).into()); }
        other => { let _ = protocol::write_message(&mut tls_stream, &RelayMessage::Ack { status: "error: expected Handshake as first message".into() }).await; return Err(format!("expected Handshake, got {:?}", other).into()); }
    };
